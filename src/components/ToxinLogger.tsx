...
export default function ToxinLogger({ onUpdate }: { onUpdate?: () => void }) {
    ...
    const risk = (count: number, threshold: number) =>
        count === 0 ? { label: "Nominal", color: "#10b981" } :
            count < threshold ? { label: "Elevated", color: "#f59e0b" } :
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
                {[
                    ...
                    <div key={type} className={`card p-3 ${count > 0 ? "border-[var(--border-subtle)]" : ""}`}>
                        <div className="flex items-center gap-2 mb-2">
                            <Icon className="w-4 h-4 text-[var(--text-muted)]" />
                            <span className="text-xs text-[var(--text-muted)] capitalize">{type}</span>
                        </div>
                        <div className="text-2xl font-bold text-[var(--text-primary)] stat-num">{count}</div>
                ...
                <div className="flex gap-2">
                    {(["smoking", "alcohol"] as ToxinType[]).map((t) => (
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all border ${selected === t ? "bg-white/8 border-[var(--border-subtle)] text-[var(--text-primary)]" : "border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-[var(--text-primary)]"
                                }`}
                        >
                            {t === "smoking" ? <Cigarette className="w-4 h-4" /> : <Wine className="w-4 h-4" />}
                            {t === "smoking" ? "Inhalation" : "Ingestion"}
                        </button>
                    ))}
                </div>

                <div className="flex items-center gap-4 justify-center py-2">
                    <button onClick={() => setQuantity(Math.max(0.5, quantity - (selected === "alcohol" ? 0.5 : 1)))} className="w-9 h-9 rounded-lg bg-white/8 hover:bg-white/15 text-[var(--text-primary)] transition-colors text-lg font-bold">−</button>
                    <div className="text-center">
                        <div className="text-3xl font-bold text-[var(--text-primary)] stat-num">{quantity}</div>
                        <div className="text-xs text-[var(--text-muted)]">{selected === "smoking" ? "units" : "units"}</div>
                    </div>
                    <button onClick={() => setQuantity(quantity + (selected === "alcohol" ? 0.5 : 1))} className="w-9 h-9 rounded-lg bg-white/8 hover:bg-white/15 text-[var(--text-primary)] transition-colors text-lg font-bold">+</button>
                </div>
